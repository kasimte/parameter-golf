...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, q_delta_fn=None, v_delta_fn=None) -> tuple[Tensor, Tensor | None]:
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, q_delta_fn=None, v_delta_fn=None) -> tuple[Tensor, Tensor | None]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, raw_v = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0, q_delta_fn=q_delta_fn, v_delta_fn=v_delta_fn)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0, q_delta_fn=qd, v_delta_fn=vd)
        ...
        x = self.final_norm(x)
        if lora is not None:
            # LoRA TTT path: return per-token loss (bsz, seq_len)
            if self.tie_embeddings:
...
class BatchedTTTLoRA(nn.Module):
    ...
    def __init__(self, bsz: int, model: GPT, rank: int):
        ...
        q_out = model.qo_bank.shape[1]  # model_dim
        v_out = model.kv_bank.shape[1]  # kv_dim
        self.lm_head_lora = BatchedLinearLoRA(bsz, dim, vocab, rank)
        self.q_loras = nn.ModuleList([BatchedLinearLoRA(bsz, dim, q_out, rank) for _ in model.blocks])
        self.v_loras = nn.ModuleList([BatchedLinearLoRA(bsz, dim, v_out, rank) for _ in model.blocks])
    def reset(self) -> None:
...
def eval_val_ttt_lora(
    ...
    log0(f"ttt_lora: {len(docs)} documents found")
    ...
    opt = torch.optim.Adam(lora.parameters(), lr=args.ttt_lora_lr, betas=(0.9, 0.95), eps=1e-10)
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        if bsz == batch_size:
            ...
            cur_opt = torch.optim.Adam(cur_lora.parameters(), lr=args.ttt_lora_lr, betas=(0.9, 0.95), eps=1e-10)
        ...
        if (bi // batch_size) % 10 == 0:
            log0(f"ttt_lora: batch {bi}/{len(rank_docs)} elapsed={time.perf_counter()-t0:.1f}s")
    ...
    log0(f"ttt_lora: done val_loss={val_loss:.4f} val_bpb={val_bpb:.4f} elapsed={time.perf_counter()-t0:.1f}s")
    return val_loss, val_bpb

# --- Legacy SGD TTT (replaced by LoRA TTT above) ---
...
def main() -> None:
    ...
    # Per-document LoRA TTT (replaces SGD TTT)
    if args.ttt_enabled:
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(f"lora_ttt val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} "
             f"eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms")
        log0(f"lora_ttt_exact val_loss:{ttt_loss:.8f} val_bpb:{ttt_bpb:.8f}")
    if distributed:
